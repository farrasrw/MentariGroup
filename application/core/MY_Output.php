...
class MY_Output extends CI_Output {
	 ...
	 * @return void
	 */
	function set_template($template_view, $data=array()){
...
//		get_instance()->session->set_flashdata("__messages", serialize($this->_messages));
	...
	private function get_template_output($output){
		...
		if(function_exists("get_instance") && class_exists("CI_Controller")){
			...
			//Session data
			$data["tempIsLogin"] 	= $ci->session->userdata('isLogin');
			$data["tempUserId"] 	= $ci->session->userdata('username');
			$data["tempChatId"] 	= $ci->session->userdata('chatid');
			
			//Additional data
			$data["templatedata"] = $this->_templatedata;
